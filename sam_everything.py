...
def main():
    ...
    combined_mask = f'borebreen_crop_drone_{str(args.file_number)}.png'                  # Combined fineal output mask
    ...
    output_dir = f'./results/everything/{args.model_type}/borebreen_image_{str(args.file_number)}_Everything/'  # Creates the file path for the output results
    ...
    # Stores the SAM model in the sam variable and sends it to the device set
    sam = sam_model_registry[args.model_type](checkpoint=args.sam_checkpoint)
    sam.to(device=device)
    
    # Creates the SAM object and stores it in the mask generator predictor variable
    ...
    # Key to be removed from the results dictionary
    ...
    for idx, arr in enumerate(list_of_integer_arrays):
        ...
        output_path = os.path.join(output_dir + output_file_1, f'borebreen_image_{str(args.file_number)}_{idx + 1}.png')
    
        # Save the array as an image using OpenCV
        cv2.imwrite(output_path, int_array)
    
    # Pritns out the output directory to save the segmentaiton mask as an png image to
    ...
    for idx, arr in enumerate(list_of_integer_arrays):
        ...
        # Define the output file path
        output_path = os.path.join(output_dir + output_file_2, f'borebreen_image_{str(args.file_number)}_{idx + 1}.png')
    
        # Save the array as an image using OpenCV
        cv2.imwrite(output_path, arr)
    
    # Pritns out the output directory to save the 0 1 mask as a png image to
    ...
    # Converts the values of the SAM model output segmenation mask
    old_value = 1
    new_value = 2
    ...
    # Displays the converted SAM model segemtnaiton mask to the screen
    plt.figure(figsize=(fig_width, fig_height))
    plt.imshow(mask)
    plt.axis('off')
